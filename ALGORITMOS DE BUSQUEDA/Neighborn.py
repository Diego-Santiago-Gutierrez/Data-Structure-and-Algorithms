...
def findClosestNeighbors(A, num, max, busqueda): #Num->Cuantos, max->Rango, busqueda->Elemento a buscar
    ...
    def distancias(idx):              #Calculamos las distancias si son menor al radio
        return abs(A[idx] - busqueda) #Si es negativo lo vuelve positivo 
    ...
    #Diferencia sirve para saber si el numero se encuentra dentro del rango 
    if leftPointer >= 0: #Si no se encuentra en el incio 
    ...
    #Si el tamaño de vecinos es menor al numero solicitado 
...
print(findClosestNeighbors(d, 2, 15, 22))